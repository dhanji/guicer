...
import com.wideplay.guicer.stat.testservices.ChildDummyService;
import com.wideplay.guicer.stat.testservices.DummyService;
import com.wideplay.guicer.stat.testservices.StatExposerTestingService;
import com.wideplay.guicer.stat.testservices.StaticDummyService;
...
public class StatsIntegrationTest {
  ...
  public final void testPublishingDuplicatedStat() {
    ...
    for (Entry<StatDescriptor, Object> entry : snapshot.entrySet()) {
      assertEquals("Unexpected value for " + entry.getKey(), Stats.DUPLICATED_STAT_VALUE, entry.getValue());
    }
  }
  ...
  public final void testPublishingUsingDifferentExposers() {
    ...
    Stats stats = injector.getInstance(Stats.class);
    ImmutableMap<StatDescriptor, Object> snapshot = stats.snapshot();
    assertEquals("Snapshot has unexpected size: " + snapshot, snapshot.size(), 8);
